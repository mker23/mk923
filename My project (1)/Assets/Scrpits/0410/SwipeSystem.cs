...
public class SwipeSystem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0)) Calculate(Input.mousePosition);                  //마우스가 Up 되었을 떼 인수로는 마우스 위치를 입력하고 함수를 호출한다.
        ...
        if (disX > 0 || disY > 0)                                   //|| => or
         {
            ...
            else
            {
                ...
                {
                    Character.transform.position += new Vector3(-1.0f, 0.0f, 0.0f);                 //뒤쪽
                }
                else
                {   
