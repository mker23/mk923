...
public class GenController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Ray cast = Camera.main.ScreenPointToRay(Input.mousePosition);           //카메라에서 3D공간 좌표를 향해서 Ray를 검출

            RaycastHit hit;                                                            //Hit 물리레이 변수 선언

            if(Physics.Raycast(cast, out hit))                            //out 인수로 hit에 Ray값 검출된 값을 넣어준다.
            {
                if(hit.collider. tag == "Ground")                       //hit 한곳의 Tag가 Ground일 때
                {
                    GameObject temp = (GameObject)Instantiate(MonsterTemp);
                    temp.transform.position = hit.point + new Vector3(0.0f, 1.0f, 0.0f);     //디버그 빨강 라인을 2초 동안 그려준다.
